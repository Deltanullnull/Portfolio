...
function typewriter() {
    if (i < txt.length) {
        ...
        if (displayTxtHtml.includes("]")) displayTxtHtml = displayTxtHtml.replace("]", "</span>");
        else displayTxtHtml += "</span>";
        displayTxtHtml += "|";
        document.getElementById("introductionText").innerHTML = displayTxtHtml;
        i++;
        setTimeout(typewriter, speed);
    } else {
        if (displayLastChar) displayTxtHtml = displayTxtHtml.slice(0, -1);
        else displayTxtHtml += "|";
...
//# sourceMappingURL=index.5ff5fb8c.js.map
